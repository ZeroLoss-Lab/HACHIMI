# -*- coding: utf-8 -*-
"""
app_for_Qwen2.5-72B.py — 多智能体 · 实时多轮协作 · 学生画像批量生成（全部字段由智能体经API产出）
...
DEBUG_FULL_RESPONSE = False  # 需要时改为 True
...
def call_llm(messages: List[Dict[str, Any]], max_tokens=900, temperature=0.95) -> str:
    ...
    try:
        ...
        dt = time.time() - t0
        print(f"[DIAG][call_llm] 调用成功，用时 {dt:.2f}s，最后一条 messages role={messages[-1]['role'] if messages else 'N/A'}")
    except Exception as e:
        print("[DIAG][call_llm] 调用 LLM 失败")
        print("  异常类型:", type(e))
        print("  异常 repr:", repr(e))
        try:
            import requests
            ...
            print("[DIAG][call_llm] 打印 HTTP 错误详情时再次异常:", repr(e2))
        print("  traceback:\n", traceback.format_exc())
        raise

    # ---- 从 data 中尽量“撸”出 content，兼容多种 Qwen/OpenAI 风格 ----
    content = None
    ...
    try:
        ...
        # 2) content 是 list（某些 Qwen 会返回 [{"type":"text","text":"..."}]）
        ...
        if not isinstance(content, str):
            if isinstance(data, dict):
                for key in ["output_text", "text", "result", "output"]:
                    if key in data and isinstance(data[key], str):
                        content = data[key]
                        break

        # 4) 仍然没有，就直接把整个 data 转成字符串（调试用）
        if not isinstance(content, str):
            content = _json.dumps(data, ensure_ascii=False)

    except Exception as e:
        print("[DIAG][call_llm] 解析返回 JSON 结构时出错:", repr(e))
        print("  data 前 400 字符:", str(data)[:400])
        content = _json.dumps(data, ensure_ascii=False)

    # 可选：调试时看看 content 长什么样
    if DEBUG_FULL_RESPONSE:
        try:
            head = content[:120].replace("\n", " ") if isinstance(content, str) else str(content)[:120]
            print("\n" + "=" * 80)
            print(f"[LLM RAW content head] {head!r}")
            print("=" * 80 + "\n")
        except Exception as e:
            print(f"[LLM RAW content head] debug print failed: {e}")
...
def try_json(text: str) -> Dict[str, Any]:
    ...
    2) 直接 json.loads；
    3) 若失败，再用正则提取最外层 { ... }。
    过程中打印少量 DIAG，但不用任何奇怪的格式说明符。
    """
    if text is None:
        print("[DIAG][try_json] 收到 None，返回空 dict")
        return {}

    raw = str(text).strip()

    if not raw:
        print("[DIAG][try_json] 收到空字符串，返回空 dict")
        return {}
    ...
    try:
        obj = json.loads(raw)
        print("[DIAG][try_json] 直接 json.loads 成功。")
        return obj
    except Exception as e:
        print(f"[DIAG][try_json] 整体 json.loads 失败: {e!r}")
        snippet = raw[:400]
        print("[DIAG][try_json] 原始文本前 400 字符:\n" + snippet)

    # 3) 用正则抓第一个 { ... } 再试
    m = re.search(r"\{.*\}", raw, flags=re.S)
    if not m:
        print("[DIAG][try_json] 未找到花括号包裹的 JSON 片段，返回空 dict")
        return {}

    sub = m.group(0)
    try:
        obj = json.loads(sub)
        print("[DIAG][try_json] 通过中间提取的 { ... } 成功解析 JSON。")
        return obj
    except Exception as e:
        print(f"[DIAG][try_json] 中间提取 json.loads 仍然失败: {e!r}")
        return {}
...
- **重要：每个学生既有优势也有不足，禁止把所有维度都写成“非常好”“明显优势”。当目标学术水平为“中/低/差”时，你负责的字段中必须明确写出若干“中/一般/较弱/较低/低”的描述，让画像像真实的人。**

【通用输出硬约束】（所有 Agent 都必须遵守）：
- 你只能输出 **一个 JSON 对象**，不能输出数组，也不能在 JSON 外面加任何解释性文字。
- 严禁使用 ```json、``` 之类的 Markdown 代码块包裹输出。
- JSON 的最外层只能包含本 Agent 负责的字段，不得增加 "id"、"学生信息"、"profile" 等任何其他键。
- 不得再嵌套一层无意义的包装对象（例如 {"学生信息": {...}} 这种结构是禁止的）。
"""
...
def agent_scholar(wb: Whiteboard, seed: str, mode: str="propose") -> Dict[str,Any]:
    ...
    生成与约束（必须）：
    - 年龄 6~18；年龄必须是一个阿拉伯数字整数，年级与年龄匹配（允许±1年跳级/留级但需与其他段落一致）；
    - 发展阶段对象必须含三键：皮亚杰认知发展阶段、埃里克森心理社会发展阶段、科尔伯格道德发展阶段；
    - 代理名格式（多音节支持）：姓 1~2 音节、名 1~3 音节，但 2 音节的姓不允许搭配 3 音节的名；每个音节为“拼音小写+声调数字(1-5)”；姓与名之间用下划线，如li3_gan4，jiang3_jie4shi2，ou1yang2_chen2fei1，yi4_yang2qian1xi3。

    【输出格式硬约束】：
    - 你只能输出一个 JSON 对象，且**顶层只能包含以下 6 个键**：
      1) "姓名"（字符串，中文名）
      2) "年龄"（整数，例如 12）
      3) "性别"（字符串，只能是 "男" 或 "女"）
      4) "年级"（字符串，例如 "六年级"、"初一"、"高二" 等）
      5) "发展阶段"（对象，包含三个子键）
      6) "代理名"（字符串，符合给定正则）
    - "发展阶段" 必须是一个对象，且**只能**包含这三个子键：
      - "皮亚杰认知发展阶段"
      - "埃里克森心理社会发展阶段"
      - "科尔伯格道德发展阶段"
    - 严禁额外增加 "id"、"学生信息" 等其他键，不允许再包一层外壳对象。
    - 不要使用 ```json 或 ``` 包裹输出。
    ...
    {{
      ...
      "发展阶段": {{
        "皮亚杰认知发展阶段": "具体运算阶段",
        "埃里克森心理社会发展阶段": "勤奋对自卑阶段",
        "科尔伯格道德发展阶段": "习俗水平"
      }},
      "代理名": "li3_xiang3"
    }}

    请仿照上面的 JSON 结构，直接输出当前学生的 JSON 对象，不要输出任何解释文字。"""
...
def agent_academic(wb: Whiteboard, seed: str, mode: str="propose") -> Dict[str,Any]:
    ...
    字段含义与内容要求（必须）：
    - "擅长科目"：非空数组，每个元素是一个学科名称（如 "语文"、"数学"）；数组内不得重复。
    - "薄弱科目"：非空数组，每个元素是一个学科名称；与 "擅长科目" 集合不相交。
    - "学术水平"：必须严格等于以下四个字符串之一：
      1) "高：成绩全校排名前10%"
      2) "中：成绩全校排名前10%至30%"
      3) "低：成绩全校排名前30%至50%"
      4) "差：成绩全校排名后50%"
    ...
    【输出格式硬约束】：
    - 你只能输出一个 JSON 对象，且**顶层只能包含以下 3 个键**：
      1) "擅长科目"
      2) "薄弱科目"
      3) "学术水平"
    - 这 3 个键必须全部出现，不得缺失，也不得增加任何其他键。
    - 严禁使用 "学生信息"、"id" 等额外外壳对象（例如 {{"学生信息": {{...}}}} 是错误的）。
...
def agent_values(wb: Whiteboard, seed: str, mode: str="propose") -> Dict[str,Any]:
    ...
    if target in [LEVELS[1], LEVELS[2], LEVELS[3]]:
        adapt = (
            "- 【七维强制分布（与系统过滤器强绑定，务必满足）】\n"
            "  - 你必须在七个维度中同时写出优点和不足，不能全部写“较高/很高”。\n"
            "  - 只允许使用这些等级词：高 / 较高 / 中上 / 中 / 较低 / 低。\n"
            "  - 当目标学术水平为“中：成绩全校排名前10%至30%”时：七个维度中**至少 1 个维度**用“中/较低/低”来描述（可以写成“中等”“中上”“较低”“偏低”等，只要能被识别为这一档）。\n"
            "  - 当目标为“低：成绩全校排名前30%至50%”时：七个维度中**至少 2 个维度**用“中/较低/低”。\n"
            "  - 当目标为“差：成绩全校排名后50%”时：七个维度中**至少 3 个维度**用“中/较低/低”。\n"
            "  - 每出现一个“较高/很高/明显优势”的描述，都要搭配至少一个“中/较低/低/需要提升”的描述，让整体既有亮点也有短板。\n"
            "  - 对每个写成“中/较低/低”的维度，都要给出简短的依据（例如具体行为、老师反馈、习惯等）。\n"
        )
    ...
    输出体裁要求（必须）：
    - "人格"：一句或数句自然语言，总结核心人格特质（如内向/外向、责任感、开放性等），保持教育场景友好。
    - "价值观"：单段连续自然语言，不得分段、不使用条目或编号。
      - 需要显式覆盖七个维度：道德修养、身心健康、法治意识、社会责任、政治认同、文化素养、家庭观念；
      - 每个维度都应出现一个可识别的等级词（如 高/较高/中上/中/较低/低），并给出简短依据。

    {adapt}

    【输出格式硬约束】：
    - 你只能输出一个 JSON 对象，且**顶层只能包含以下 2 个键**：
      1) "人格"
      2) "价值观"
    - 不允许出现 "学生信息"、"id"、"评价" 等任何其他顶层键。
    - "价值观" 必须是单段文本，中间不要出现空行、列表符号（如 "-"、"•"、"1." 等）或 Markdown。
    - 不要使用 ```json 或 ``` 包裹输出。

    【合格示例（结构示意，内容可不同）】：
    {{
      "人格": "性格温和细腻，有较强的责任感和同理心，做事稳重但在陌生情境中略显拘谨。",
      "价值观": "在道德修养上整体较高，能主动遵守班级规则并在同伴发生冲突时尝试调解；身心健康处于中上水平，能通过运动和写日记调节情绪；法治意识较为清晰，能够用简单的法律和校规语言理解对错边界；社会责任感中等偏上，愿意在班级和社区活动中承担任务但偶尔需要教师提醒；在政治认同方面能理解并认同国家和集体的基本价值与象征；文化素养较高，阅读面广，对历史与文学作品保持好奇；家庭观念稳定，以父母为重要支持来源，能在尊重父母意见的同时尝试表达自我。"
    }}

    请仿照上面的 JSON 结构，只输出 JSON 对象。"""
...
def agent_social_creative(wb: Whiteboard, seed: str, mode: str="propose") -> Dict[str,Any]:
    ...
    if target in [LEVELS[1], LEVELS[2], LEVELS[3]]:
        adapt = (
            "- 【八维等级强制分布（与过滤器强绑定）】\n"
            "  - 你需要先在脑中为 8 个维度分别选定一个等级词，只能从“高/较高/中上/中/较低/低”中选择，然后再用一段话把这 8 个维度串联起来描述。\n"
            "  - 禁止 8 个维度全部写成“高/较高”，必须有明显的起伏，像真实学生那样有强项也有弱项。\n"
            "  - 当目标学术水平为“中”时：8 个维度中**至少 2 个维度**用“中/较低/低”这类词（可以写成“中等”“中上”“稍显不足”“偏低”等，但层次要清晰）。\n"
            "  - 当目标为“低”时：8 个维度中**至少 3 个维度**用“中/较低/低”。\n"
            "  - 当目标为“差”时：8 个维度中**至少 4 个维度**用“中/较低/低”，整体创造力应呈现明显偏弱但仍有可发展的空间。\n"
            "  - 若“可行性”写成“较低/低”，则“提出方案”的等级不允许高于“中”（不能写成“高/较高/中上”）。\n"
            "  - 结尾的“雷达总结”要明确点出哪些维度是优势，哪些维度是明显需要提升的短板。必须含有“雷达”或者“总结”词。\n"
        )
    ...
    字段与体裁要求（必须）：
    - "社交关系"：单段（约 160~260 字），按“背景 → 关键事件 → 影响”的顺序叙述，不得换行或条列。
    - "创造力"：单段自然语言，且必须包含：
      - 对八个维度逐一评价：流畅性、新颖性、灵活性、可行性、问题发现、问题分析、提出方案、改善方案，每一维都有清晰的等级词（高/较高/中上/中/较低/低）和简短依据；
      - 结尾有一个整体“雷达总结”的综合概括。
    - 若“可行性”较低或低，则“提出方案”不高于中等，避免自相矛盾。
    {adapt}
    ...
    - 你只能输出一个 JSON 对象，且**顶层只能包含以下 2 个键**：
      1) "社交关系"
      2) "创造力"
    - 不允许出现 "学生信息"、"id"、"描述" 等任何其他键。
    - "社交关系" 与 "创造力" 都必须是单段文本，不得包含列表符号、编号、Markdown 等。
    - 不要使用 ```json 或 ``` 包裹输出。
...
def agent_health(wb: Whiteboard, seed: str, mode: str="propose") -> Dict[str,Any]:
    ...
    if target in [LEVELS[2], LEVELS[3]]:
        adapt = (
            "- 【心理指数分布（与过滤器强绑定）】\n"
            "  - 请在文中显式给出四个项目的等级或程度：综合心理状况、幸福指数、抑郁风险、焦虑风险。\n"
            "  - 当目标学术水平为“低/差”时：\n"
            "    · 综合心理状况 和 幸福指数 中**至少有 1 项**使用“中/一般/较低/偏低”等中低水平表述，不能两项都写“较高/很高”。\n"
            "    · 抑郁风险 与 焦虑风险 不能同时写成“低/很低/几乎没有风险”，至少有 1 项需要体现出“轻度/有一定可能/需要关注”这类表述。\n"
            "  - 同时要保持教育场景的非诊断化语气：避免直接使用“重度抑郁/双相/需要住院”等临床诊断用语，用“可能存在…倾向”“阶段性情绪低落”“建议进一步评估”等方式描述。\n"
            "  - 在描述背景故事和支持系统时，要让读者感觉到：虽然目前存在一定压力或困扰，但通过家庭、教师、学校资源的配合，是可以逐步改善的。\n"
        )
    ...
    - "心理健康"：单段自然语言，不得分段或条列。
      ...
      1) 概述整体心理状态；
      2) 至少两点性格特征，与心理适应相关；
      3) 对以下项目给出明确等级或程度表述：综合心理状况、幸福指数、抑郁风险、焦虑风险；
      4) 如无明确心理疾病，需出现“信息不足或未见显著症状”等非诊断化表述；如有风险或倾向，用“可能存在…倾向”“轻度…体验”“建议进一步评估”等；
      5) 简短背景故事（如学习压力、人际冲突、家庭事件等）；
      6) 目前的支撑与应对方式（家庭、老师、同伴、学校资源等）。
    {adapt}

    【输出格式硬约束】：
    - 你只能输出一个 JSON 对象，且**顶层只能包含 1 个键**：
      1) "心理健康"
    - 不允许出现 "学生信息"、"id"、"评价" 等其他键。
    - "心理健康" 必须是单段文本，不得包含空行、列表符号、Markdown 代码块。
    - 不要使用 ```json 或 ``` 包裹输出。
...
class Orchestrator:
    ...
    def run_one(self, sid: int, sampling_hint: Optional[Dict[str, Any]] = None,
        ...
        # ★ 临时调试：打印这次成品的完整内容
        try:
            print("[DEBUG][run_one final]", json.dumps(final, ensure_ascii=False)[:8000])
        except Exception as _e:
            print("[DEBUG][run_one final] 打印失败：", repr(_e))
...
def _worker_process_one(orch: Orchestrator, sid: int, slot: Dict[str, Any],
                        ...
                        ) -> Tuple[bool, Optional[Dict[str,Any]], Optional[List[Dict[str,str]]], str]:
    set_client_for_thread(client)
    last_dialog = None
    last_err = ""
    ...
    for attempt in range(1, max_retries+1):
        try:
            ...
            if not ok:
                last_err = "轻量过滤不通过：" + "; ".join(reasons)
                print(f"[DIAG][_worker_process_one] sid={sid} attempt={attempt} 轻量过滤失败: {last_err}")
                continue
            ...
            if not sim_gate.try_accept(key_text):
                ...
                print(f"[DIAG][_worker_process_one] sid={sid} attempt={attempt} 相似度过高: {last_err}")
                continue
            ...
            last_err = f"尝试{attempt}/{max_retries}失败：{e!r}"
            print(f"[DIAG][_worker_process_one] sid={sid} attempt={attempt} 发生异常: {e!r}")
            print("[DIAG][_worker_process_one] Traceback:\n" + traceback.format_exc())
        finally:
            if 'dialog' in locals():
                last_dialog = dialog

    print(f"[DIAG][_worker_process_one] sid={sid} 在 {max_retries} 次尝试后仍失败。最后错误原因：{last_err}")
...
if st.session_state.running:
    ...
    if not st.session_state.paused:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as ex:
            ...
            st.session_state.last_error = None if accepted == len(scheduled_sids) else (
                f"本步并行完成 {accepted}/{len(scheduled_sids)}，失败已回队重试；"
                f"最后一个失败原因：{last_error}"
            )
...
# ----------- 调试面板：无论是否在运行都展示 -----------
with console:
    st.subheader("🔍 运行诊断（出问题时请把这一块截图/复制给我）")

    col1, col2 = st.columns(2)
    with col1:
        st.write("running:", st.session_state.running)
        st.write("paused:", st.session_state.paused)
        st.write("total_n:", st.session_state.total_n)
        st.write("chunk_size:", st.session_state.chunk_size)
        st.write("chunks_total:", st.session_state.chunks_total)
        st.write("next_sid（当前最小缺口）:", st.session_state.next_sid)
        st.write("已完成条数 len(completed_sids):", len(st.session_state.completed_sids))
        st.write("待生成条数 len(pending):", len(st.session_state.pending))
        if st.session_state.run_dir:
            st.write("run_dir:", st.session_state.run_dir)

    with col2:
        st.write("last_error（最近一批并行的错误汇总）:")
        if st.session_state.last_error:
            st.error(st.session_state.last_error)
        else:
            st.success("目前 last_error 为空（没有被记录的批量错误）")

        # 展示一个示例 slot，确认调度配额是不是正常
        if st.session_state.sid2slot:
            try:
                example_sid = min(st.session_state.sid2slot.keys())
                st.write("示例 slot（最小 sid 对应的采样约束）：")
                st.json({"sid": example_sid, "slot": st.session_state.sid2slot[example_sid]})
            except Exception as e:
                st.write("展示示例 slot 时出错:", repr(e))

    # 最近一次成功写入的画像
    if st.session_state.last_item:
        with st.expander("✅ 最近一次成功通过过滤并写盘的画像（last_item）", expanded=False):
            st.json(st.session_state.last_item)

    # 最近一次协作日志（白板讨论）
    if show_console and st.session_state.last_dialog:
        with st.expander("🗒 最近一次协作日志（last_dialog，截尾 30 条）", expanded=False):
            for turn in st.session_state.last_dialog[-30:]:
                speaker = turn.get("speaker", "?")
                content = turn.get("content", "")
                st.markdown(f"**{speaker}**：{content}")
    elif show_console:
        st.info("还没有可展示的协作日志（可能一个样本都还没成功完成）。")
